...
public class AuthExceptionHandler {
  ...
  public ResponseEntity<ErrorResponseDTO> handleEmailTaken() {
    ...
    return ResponseEntity.status(status).body(ErrorResponseDTO.of(status,"Email address is already in use"));
  }
  ...
  public ResponseEntity<ErrorResponseDTO> handleInvalidSecret() {
    ...
    return ResponseEntity.status(status).body(ErrorResponseDTO.of(status,"Invalid user secret provided"));
  }
  ...
  public ResponseEntity<ErrorResponseDTO> handleValidation(MethodArgumentNotValidException ex) {
    HttpStatus status = HttpStatus.BAD_REQUEST;
    return ResponseEntity.status(status)
        .body(ErrorResponseDTO.of(status,
  ...
  public ResponseEntity<ErrorResponseDTO> handleGeneralAuthError(Exception ex) {
    ...
    return ResponseEntity.status(status)
        .body(ErrorResponseDTO.of(status,"Unknown authentication error occurred: " + ex.getMessage()));
  }
}
