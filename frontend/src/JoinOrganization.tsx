...
export default function JoinOrganization() {
    const [form, setForm] = useState({
        name: '', surname: '', emailAddress: '', password: '', jobTitle: '', organizationToken: ''
    });
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        alert(res.ok ? 'Successfully joined!' : 'Error joining organization');
    };
    ...
    return (
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', width: '250px', gap: '10px' }}>
            ...
            <input placeholder="Organization Token" required onChange={e => setForm({...form, organizationToken: e.target.value})} />
